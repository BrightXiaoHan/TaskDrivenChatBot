...
from utils.define import MODEL_TYPE_DIALOGUE  # noqa: E402
...
try:
    manager.checkout(
        params["robot_code"], MODEL_TYPE_DIALOGUE, params["dialogue_version"]
    )
except Exception:
...
async def run():
    ...
    data = await manager.session_reply(
        params["robot_code"], sessionId, user_says, "user1", params["params"]
    )
    ...
    while True:
        ...
        if user_says == "info":
            print(
                manager.agents[params["robot_code"]].user_store[sessionId]._latest_msg()
            )
